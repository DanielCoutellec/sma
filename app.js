...
app.get("/courses2026", (req, res) => {
  ...
  const courses = JSON.parse(
    fs.readFileSync(
      path.join(process.cwd(), "data", "courses2026.json"),
      "utf8"
    )
  );
  ...
  // ordre des mois FR (on accepte "MARS 26", "MARS", etc.)
  ...
  // tableau de groupes dans l’ordre des mois
  const groupedCourses = Array.from(groupsMap.entries()).sort(
    ([moisA], [moisB]) => monthIndex(moisA) - monthIndex(moisB)
...
app.get("/courses2026", (req, res) => {
  try {
    const file = path.join(__dirname, "data", "courses2026.json");
    ...
    const monthIndex = (mois) => {
      ...
      if (m.includes("avr")) return 4;
      if (m.includes("mai")) return 5;
      if (m.includes("jui") && !m.includes("juil")) return 6;
      if (m.includes("juil")) return 7;
      if (m.includes("aou") || m.includes("aoû")) return 8;
      if (m.includes("sep")) return 9;
      if (m.includes("oct")) return 10;
    ...
    // Nettoyage minimal (évite les undefined)
